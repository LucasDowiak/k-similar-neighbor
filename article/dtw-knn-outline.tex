...
We will compare the returns to a trading strategy strategy discussed in Gatev et. al. This strategy consists of the following steps:
1. Pair stocks together that are highly associated with each other.
2. Take advantage of any large temporary cumulative return deviations and bet on a likely re-alignment of the cumulative returns in the near future
3. Close out any open positions at the end of the trading window

There are three different propsed methods for chosing the association measure:
1. Use the un-treated returns. Measure closeness via correlation. Use static/unconditional distributions and a two standard deviation threshold on the cumulative return differential as a signal to open positions.
2. Filter the returns through an ARIMA-GARCH model. Measure closeness on the fitted residuals. Use rolling mean/variance values to calculate adjusted threshold during trading period
3. Calculate the DTW metric between the cumulative returns of the pairs. Use synchronous L1 value and the two standard deviation signal to open positions.
