...
One way to compare these measures is putting them to practical use. In this section a simple trading strategy is described that centers around finding "similar" pairs of stocks and trading off the expectation that any major short term deviations in their prices are eventually unwound over a longer time horizon. If the returns to using dynamic time warping to form trading pairs are materially different than returns from a correlation based approach it could help us understand whether dtw can provide complimentary information to use in combination with correlation. The specifics of this pair trading strategy are the same as the approach taken by \cite{Gatev_et_al_2006} with a few adjustments made to take into account the unique nautre of dynamic time warping and its difference to correlation as a measure of similarity. This backtesting experiment is not carried out because of high expected returns, especially in the context of our modern trading environment. The rise of high frequency trading in the 1990's and 2000's has created the financial infrastructure that sees arbitrage opportunities resolved in microseconds\footcite[See][for a more detailed ]{Aquilina_et_al_2021} while this article studies asset returns at a daily frequency. In addition \cite{Gatev_et_al_2006} was published close to two decades ago in a widely read journal making its recipe part of the established financial canon. The benefits to using this particular trading strategy is that it provides a suitable context to directly compare the impact of using dynamic time warping -- instead of correlation -- to identify the "similar" pairs to trade.

Given a single stock-pair in a trader's portfolio a buy-signal is trigged if the price deviation between the two stocks exceeds a certain threshold. Once triggered the trader goes long in the lower priced stock and short in the higher priced stock. The positions are unwound when the prices come back to parity, profits are recorded, and the trader stands ready to open another position given a new buy-signal. At the end of the trading period all open positions are closed at the prevailing price. To summarize, the general strategy has the following sequence of steps:

\begin{enumerate}
    \item Set length of time to use for pair selection and cross-validation for optimal buy-signal thresholds. Set length of time for the execution of the trading strategy. In this article a one year duration is used for both pair selection and trade execution periods.
    \item Standardize all stock prices at the beginning of the pair selection and trading period intervals.
    \item Pair Selection: Use recent past price movements to identify the top N pairs of stocks that are the most \textit{similar}. Use the same period to find the optimal buy-signal threshold via cross-validation.
    \item Trading Period: Execute the strategy for each of the N stock pair in the portfolio. Close out any open positions at the end of the trading period.
\end{enumerate}
...
\begin{table}[htb]
    ...
    \begin{tabular}{l r r l r r r r r r}
        ...
        \vspace{-1mm} \\
        101-200   & unadjusted cor &  0.0047 & 0.0015{$**$}    & 0.019 &  0.0058 &  0.205 & 2.592 & -0.029 & 0.044 \\
        101-200   & model cor      &  0.0015 & 0.0020          & 0.022 &  0.0073 & -1.242 & 5.263 & -0.066 & 0.034 \\
        101-200   & dtw            & -0.0044 & 0.0057          & 0.042 & -0.0012 & -1.043 & 3.218 & -0.110 & 0.040 \\
        \vspace{-1mm} \\
        1001-1100 & unadjusted cor & -0.0074 & 0.0031{$^{*}$}  & 0.033 & -0.0018 & -0.337 & 2.962 & -0.083 & 0.060 \\
        1001-1100 & model cor      &  0.0005 & 0.0039          & 0.026 & -0.0014 &  1.276 & 6.203 & -0.044 & 0.085 \\
        1001-1100 & dtw            & -0.0027 & 0.0071          & 0.042 &  0.0057 & -0.085 & 2.294 & -0.086 & 0.065 \\
        \vspace{-1mm} \\
        \hline
        \vspace{1 mm} \\
        ...
        \vspace{-1mm} \\
        101-200   & unadjusted cor &  0.0097 & 0.0023{$^{**}$} & 0.031 &  0.0088 &  0.139 & 2.066 & -0.037 & 0.065 \\
        101-200   & model cor      &  0.0046 & 0.0024{$+$}     & 0.032 &  0.0128 & -0.600 & 2.913 & -0.076 & 0.047 \\
        101-200   & dtw            & -0.0045 & 0.0058          & 0.042 & -0.0012 & -1.046 & 3.222 & -0.111 & 0.041 \\
        \vspace{-1mm} \\
        1001-1100 & unadjusted cor & -0.0118 & 0.0060{$^{*}$}  & 0.050 & -0.0023 & -0.318 & 2.235 & -0.113 & 0.075 \\
        1001-1100 & model cor      & -0.0028 & 0.0058          & 0.045 & -0.0020 & -0.293 & 4.727 & -0.125 & 0.107 \\
        1001-1100 & dtw            & -0.0031 & 0.0071          & 0.043 &  0.0058 & -0.080 & 2.253 & -0.087 & 0.065 \\
        \vspace{-1mm} \\
        \hline
        \vspace{1 mm} \\
        ...
        \vspace{-1mm} \\
        101-200   & unadjusted cor & 0.0739 & 0.0219{$^{**}$} & 0.210 & 0.1129 & -0.890 & 3.521 & -0.496 & 0.362 \\
        101-200   & model cor      & 0.0907 & 0.0255{$^{**}$} & 0.216 & 0.1432 & -0.719 & 3.599 & -0.505 & 0.427 \\
        101-200   & dtw            & 0.0840 & 0.0181{$^{**}$} & 0.165 & 0.1309 & -1.073 & 4.193 & -0.388 & 0.293 \\
        \vspace{-1mm} \\
        1001-1100 & unadjusted cor & 0.0756 & 0.0281{$^{**}$} & 0.214 & 0.1423 & -1.033 & 3.905 & -0.522 & 0.362 \\
        1001-1100 & model cor      & 0.0655 & 0.0280{$^{*}$}  & 0.205 & 0.0941 & -0.982 & 3.869 & -0.500 & 0.333 \\
        1001-1100 & dtw            & 0.0954 & 0.0170{$^{**}$} & 0.171 & 0.1381 & -1.014 & 4.276 & -0.402 & 0.326 \\
        \vspace{-1mm} \\
        \hline
    \end{tabular}
    \caption{Average Annual Return Distribution}
    \begin{tablenotes}
        \item{\footnotesize Three different measures of portfolio returns are summarized in this table. The top section records committed returns where the total return of the portfolio during the trading period is divided by the number of pairs selected to trade. Section II of the table records fully invested returns where the total return of the portfolio during the trading period is devided by the number of pairs selected to trade. Section III records the annual return of the portfolio if the trader simply goes long in at the beginning of the trading period.}
...
\begin{table}[!htb]
    ...
    \begin{tabular}{l r r r r r r r r r r r}
          & \rot{Communication Services} & \rot{Consumer Discretionary} & \rot{Consumer Staples} & \rot{Energy} & \rot{Financials} & \rot{Health Care} & \rot{Industrials} & \rot{Information Technology} & \rot{Materials} & \rot{Real Estate} & \rot{Utilities} \\
        \textbf{Dynamic Time Warping} &    &    &    &    &    &    &    &    &    &    &    \\
        Communication Services   & 14 &  . &  . &  . &  . &  . &  . &  . &  . &  . &   . \\
        Consumer Discretionary   &  . &  3 &  . &  . &  . &  . &  . &  . &  . &  . &   . \\
        Consumer Staples         &  2 &  3 & 13 &  . &  . &  . &  . &  . &  . &  . &   . \\
        Energy                   &  . &  . &  1 &  5 &  . &  . &  . &  . &  . &  . &   . \\
        Financials               &  2 &  2 & 13 &  . & 50 &  . &  . &  . &  . &  . &   . \\
        Health Care              &  3 &  . & 12 &  1 & 12 &  6 &  . &  . &  . &  . &   . \\
        Industrials              &  3 &  4 &  7 &  3 &  8 &  6 & 20 &  . &  . &  . &   . \\
        Information Technology   &  2 &  1 &  1 &  2 &  9 &  3 &  6 &  2 &  . &  . &   . \\
        Materials                &  . &  3 &  1 &  2 &  4 &  1 &  6 &  2 &  2 &  . &   . \\
        Real Estate              &  . &  1 &  1 &  . &  2 &  . &  2 &  . &  2 & 49 &   . \\
        Utilities                &  4 &  4 & 25 &  2 & 11 &  9 &  6 &  1 &  3 &  9 & 178 \\
        \vspace{1 mm} \\
        \textbf{Model Correlation}    &    &    &    &    &    &    &    &    &    &    &    \\
        ...
        \hline
    \end{tabular}
    \caption{Sector Distribution of Top 25 Portfolio Pairs}
    \begin{tablenotes}
        \item{\footnotesize Footnote size table note}
    \end{tablenotes}
\end{table}
